...
export default function ProductDetail() {
  ...
  const productDatabase = {
    'ax-90': {
      id: 'ax-90', name: 'AX-Series Distribution Hub', category: 'Lighting', price: 4250.00, catalogUrl: '/catalogs/ax-series-hub.pdf',
    ...
    'lum-tx500': {
      id: 'lum-tx500', name: 'Titan High-Bay Luminaire', category: 'Lighting', price: 850.00, catalogUrl: '/catalogs/titan-high-bay.pdf',
  ...
  useEffect(() => {
    const handleIdChange = () => {
      ...
      const related = Object.values(productDatabase).filter(item => item.category === foundProduct.category && item.id !== foundProduct.id)
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 lg:gap-12 items-start mb-24">
          ...
          <div className="lg:col-span-5 space-y-10 lg:sticky lg:top-32">
            <div className="space-y-4">
              <span className="inline-block bg-primary/10 text-primary px-3 py-1 font-label text-[10px] font-bold tracking-[0.2em] uppercase">{product.category}</span>
              <h1 className="text-4xl md:text-5xl font-headline font-black tracking-tighter leading-none text-primary uppercase italic">{product.name}</h1>
              <p className="text-secondary/80 leading-relaxed font-body text-lg max-w-xl">{product.longDescription}</p>
            </div>
            <div className="space-y-6">
              <div>
                <p className="text-tertiary font-label text-[10px] tracking-widest uppercase font-black mb-1">{t('productDetail.pricePoint')}</p>
                <p className="text-4xl font-headline font-black text-primary">EGP {product.price.toLocaleString('en-EG', { minimumFractionDigits: 2 })}</p>
              </div>
              <div className="flex flex-col gap-3">
                <button onClick={() => navigateToProduct(`quote-${product.id}`)} className="w-full bg-primary text-white py-5 text-xs font-headline font-bold uppercase tracking-[0.2em] flex items-center justify-center gap-4 hover:bg-primary/90 transition-all">
                  {t('productDetail.requestQuote')}
                  <span className="material-symbols-outlined text-sm">arrow_forward</span>
                </button>
                <a href={product.catalogUrl} download className="w-full border border-outline-variant text-primary py-4 text-[10px] font-headline font-bold uppercase tracking-[0.15em] flex items-center justify-center gap-3 hover:bg-surface-container-high transition-all">
              ...
              {relatedProducts.map((item) => (
                <div key={item.id} onClick={() => navigateToProduct(item.id)} className="group cursor-pointer">
                  <div className="aspect-[4/3] bg-surface-container-low overflow-hidden mb-5 border border-outline-variant/10">
                    <img src={item.mainImage} alt={item.name} className="w-full h-full object-cover grayscale group-hover:grayscale-0 group-hover:scale-105 transition-all duration-700" />
                  </div>
                  <p className="text-[9px] font-bold tracking-widest uppercase text-tertiary mb-1">{item.category}</p>
                  <h3 className="text-lg font-headline font-bold text-primary leading-tight group-hover:text-primary/70 transition-colors uppercase">{item.name}</h3>
                  <p className="text-sm font-body mt-2 text-secondary font-bold">EGP {item.price.toLocaleString('en-EG')}</p>
                </div>
              ))}
            </div>
          </section>
