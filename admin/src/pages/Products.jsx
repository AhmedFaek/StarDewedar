            ...
            {products.map((item) => (
              <tr key={item.id} className="hover:bg-surface-container-low transition-colors">
                ...
                <td className="px-8 py-6 text-right">
                  <button onClick={() => openEditModal(item)} className="text-tertiary hover:text-primary mr-4 uppercase text-xs font-black">Edit</button>
                  <button className="text-error uppercase text-xs font-black">Delete</button>
                </td>
              </tr>
            ))}
          </tbody>
          ...
          <div className="bg-surface-container-lowest border border-surface-variant w-full max-w-3xl max-h-[90vh] overflow-y-auto shadow-2xl animate-in zoom-in duration-200">
            <form onSubmit={handleSave}>
                ...
                <div className="pt-4 border-t border-surface-variant">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                    ...
                    <div className="bg-surface-container-low border border-surface-variant p-4 h-32 overflow-y-auto">
                      ...
                      {/* Example of an attached catalog */}
                      <div className="flex items-center justify-between bg-surface-container-lowest p-2 border border-surface-variant mb-2">
                        <span className="text-[10px] font-bold text-primary truncate">MAINTENANCE_GUIDE_V2.PDF</span>
                        <button type="button" className="text-error material-symbols-outlined text-sm">delete</button>
                      </div>
                    </div>
                  </div>
                </div>
                ...
                <div className="pt-4 border-t border-surface-variant">
                  ...
                  <div className="space-y-2">
                    <div className="flex gap-2">
                      ...
                      <button type="button" className="text-error material-symbols-outlined">delete</button>
                    </div>
                  </div>
                </div>
              </div>
              ...
              <div className="px-8 py-6 bg-surface-container-low border-t border-surface-variant flex justify-end gap-4 sticky bottom-0">
                <button type="button" onClick={() => setIsModalOpen(false)} className="text-xs font-bold uppercase tracking-widest text-secondary px-6">Cancel</button>
